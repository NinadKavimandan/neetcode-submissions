class Solution {
public:
    bool isCyclic (vector<vector<int>>& adjList, int node, int parent, vector<bool>& visited)
    {
        bool response = false;
        visited[node] = true;
        ...
        for (int next : adjList[node])
        {
            ...
            {
                response &= isCyclic(adjList, next, node, visited);
            }
        }
    ...
    vector<int> findRedundantConnection(vector<vector<int>>& edges)
    {
        ...
        for (vector<int>& edge : edges)
        {
            ...
            if (isCyclic(adjList, edge[0] - 1, -1, visited))
            {
                return edge;
            }
        }
